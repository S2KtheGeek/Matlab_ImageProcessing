...
d = sqrt(x.^2+y.^2);
h=d<=d0;%value should be less than equal to the thresh hold d0
af1= af.*h;%application of ideal low pass filter
af11 = ifft2(af1);%applying inverse fourier transform
subplot(2,2,1); imshow(a);title('original image');
subplot(2,2,2); fftshow(af); title('Fourier Transform');
subplot(2,2,3); fftshow(af1); title('Fourier Transform ILPF');
subplot(2,2,4); ifftshow(af11); title('Inv-Fourier Transform ILPF');